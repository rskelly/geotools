...
size_t buildGrid(const std::string& infile, double bounds[6], double res, int& cols, int& rows, std::vector<double>& grid) {
...
bool normalizeRaster(const std::string& infile, std::vector<double>& grid, double* bounds,
	...
	while(rdr.next(x, y, z, cls)) {
		...
		int col0 = col;
		int row0 = row;
		double cx = bounds[0] + col * res + 0.5 * res;
		double cy = bounds[1] + row * res + 0.5 * res;
		double z = grid[row * cols + col];
		double z0 = -9999;
		double z1 = -9999;
		int ci = x < cx ? -1 : 1;
		int cl = x < cx ? 0 : cols - 1;
		int ri = y < cy ? -1 : 1;
		int rl = y < cy ? 0 : rows - 1;
		col0 += ci;
		do {
			if((z0 = grid[row * cols + col0]) != -9999)
				break;
			col0 += ci;
		} while(col0 != cl);
		row0 += ri;
		do {
			if((z1 = grid[row0 * cols + col]) != -9999)
				break;
			row0 += ri;
		} while(row0 != rl);
		if(z0 == -9999) z0 = z;
		if(z1 == -9999) z1 = z;
		double cx0 = bounds[0] + col0 * res + 0.5 * res;
		double cy0 = bounds[1] + row * res + 0.5 * res;
		double cx1 = bounds[0] + col * res + 0.5 * res;
		double cy1 = bounds[1] + row0 * res + 0.5 * res;
		double zi = bary(x, y, cx, cy, z, cx0, cy0, z0, cx1, cy1, z1);
		rdr.setZ(zi);
		if(++n % 100000 == 0)
			std::cout << n << " of " << rdr.size() << " (" << ((float) n / rdr.size()) << "%)\n";
	}
